...
const refresh = Effect.gen(function* () {}).pipe(
  // ...
  Effect.withSpan("refresh") // <- Effect automatically establishes a parent-child relationship - just nest withSpans!
);
```
...
class Session extends Effect.Service<Session>()("Session", {
  ...
  // Layer<A, E, R> represents a recipe to build service A, possibly failing with error E, requiring dependencies R
  // It's basically an Effect.Effect<A, E, R> for constructors - you can retry layers, merge them, and more.
  effect: Effect.gen(function* () {
    ...
    // The returned object defines the shape of the Session service in the Context
    return {
      refreshSession,
      getTokens
    };
  ...
  "CustomHttpClient",
  {
    ...
    effect: Effect.gen(function* () {
      ...
      return (yield* HttpClient.HttpClient).pipe(
        ...
        // HttpClient also comes with HttpClient.retryTransient, which retries on transient errors (RequestErrors) exponentially
        HttpClient.retry({
          times: 1,
          while: (error) => error._tag === "ForceRetryError"
        }),
...
const OtherImpl = Layer.effect(
  CustomHttpClient, // The Tag of the
  // service
  Effect.gen(function* () {
...
That's what makes Effect so powerful - the `R` type parameter isn't just for services, but for any contextual dependencies. For example, you could define a Tag for an HTTP request, allowing your endpoints to access the incoming request without passing it through every function call. You could even use the `R` channel to implement permission policies, where certain computations explicitly require specific permissions in their type signature. This way, consumers know exactly what they need to provide, and you can enforce these requirements at runtime with policy guards while maintaining clean type signatures. These are more advanced patterns that we can explore another time, but they showcase how Effect's design enables sophisticated dependency management without sacrificing type safety or code clarity.
