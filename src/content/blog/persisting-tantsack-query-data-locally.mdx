...
This works for validation, but persisting to localStorage is more nuanced, as you'd need another schema for the transformed data:
  ...
  export const set = (updater: (updater: Result | undefined) => Result | undefined) =>
    Effect.gen(function* () {
      const data = yield* Effect.sync(() =>
        Option.fromNullable(queryClient.setQueryData<Result>(makeQueryKey(), updater))
      );
