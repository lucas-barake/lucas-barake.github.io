...
If you're not familiar with [Effect Schema](https://effect.website/docs/schema/introduction/), think of it as an evolution of validators like Zod or Yup. While most validators only transform `unknown` into your type `T`, Effect Schema provides bi-directional transformation - allowing you to go from `From` to `To` and back again. This capability enables us to:

- Eliminate version management and migrations
- Let schema changes automatically invalidate cache entries
- Have invalid/stale data trigger new queries automatically

So, to give you an example, let's say we have the following response from our API:
...
Now, suppose you need to rename `created_at` to `createdAt` and transform that `string` timestamp into a proper `Date` object. With Zod, you might write something like this:
...
type TransformedTodo = z.infer<typeof todoSchema>;
```

Where we take `unknown` and validate and transform it into `TransformedTodo`.

This works for validation, but persisting to a storage is more nuanced, as you'd need another schema for the transformed data:

```ts
const transformedTodoSchema = z.object({
  id: z.string(),
  title: z.string(),
  status: z.nativeEnum(TodoStatus),
  createdAt: z.date()
...
But you're not accounting for the fact that you'd have to serialize and deserialize the JSON data. So, for instance, instead of `createdAt: Date`, you'd have to do `createdAt: string` and apply the transformation from `string` to `Date`:

```ts
const transformedTodoSchema = z
  .object({
    id: z.string(),
    title: z.string(),
    status: z.nativeEnum(TodoStatus),
...
This becomes cumbersome very quickly, and you can imagine how this would scale.

## Effect Schema to the Rescue

This is where Effect Schema really shines. Instead of having to maintain two separate schemas and manually handle serialization/deserialization, we can define a single schema that handles both the validation and transformation in both directions:
...
- Unifies validation and serialization in a single schema definition
- Automatically invalidates cached data when schemas change
- Provides complete type safety throughout the application
...
Side note: you could have your own custom `decode` and `encode` functions:
...
However, since Effect Schema comes with a lot of built-in transformations, such as `pluck`, `rename`, etc, there's no need to manually define these transformations (at least for this example, though your requirements may be different).
...
class MyHttpClient extends Effect.Service<MyHttpClient>()("MyHttpClient", {
  effect: Effect.gen(function* () {
    ...
    // spotlight-start
    const makeCachedRequest = <QK extends QueryKey, Result, Input>({
      queryKey,
      schema,
      request
    }: {
      ...
      Effect.gen(function* () {
        const stringifiedQueryKey = yield* Effect.try(() => JSON.stringify(queryKey)).pipe(
          Effect.withSpan("JSON.stringify")
        );
...
There are two approaches you can take when it comes to setting cached data:

- Implementing `QueryPersister.set` within the namespace's exported `set` function.
- Creating a `useSyncLocal` hook for each query to periodically synchronize local data with in-memory data, treating the latter as the source of truth.

For simplicity, let's go with the first approach:
...
export namespace TodosQuery {
  ...
  export const set = (updater: (updater: Result | undefined) => Result | undefined) =>
    Effect.gen(function* () {
      const data = Option.fromNullable(queryClient.setQueryData<Result>(makeQueryKey(), updater));

      const stringifiedQueryKey = JSON.stringify(makeQueryKey());

      if (Option.isSome(data)) {
        yield* QueryPersister.store(stringifiedQueryKey, yield* Schema.encode(Result)(data.value));
      } else {
...
> If your queries have a high frequency of updates, you might want to consider either debouncing the `set` function or exploring the second approach mentioned above.

### Mutations

For mutations, the idea is the same as with queries. You can create a schema for the mutation's input:
...
class Input extends Schema.Class<Input>("Input")({
  ...
  dueDate: Schema.DateTimeUtc.pipe(
    Schema.filter(DateTime.greaterThanOrEqualTo(DateTime.add(DateTime.unsafeNow(), { days: 1 })))
  )
}) {}
```

And then you can use `IndexedDB` with the mutation's key to `encode` and store the input data:
...
Notice how we use a `withPermits(1)` to ensure that we prevent race conditions.

With this, you can now export a `useSyncLocal` hook that will synchronize the stored input with the server, and a `useLocal` hook that will let you access the stored input (ideally, the hook would return the optimistic items so that you can render them in your UI alongside with the actual query - this could allow the user to see the changes even when offline, and also to possibly delete/edit them on command).

### Conclusion

Great! With this approach, your users can still interact with your application even when offline (as long as they have cached data).

Here's what I recommend doing next:

- Implement a `Connection` service to manage `persistedRequest` based on connection status changes.
- Extend the caching strategy to mutations by encoding inputs and implementing a system to decode and execute stored mutations when the application starts or regains connectivity. This will also let you automatically invalidate stale mutations.
- Create helper functions to abstract the logic for persisting and invalidating data for each query/mutation.

While this might feel somewhat manual (and potentially overwhelming), the benefit is complete control over your data layer. You can apply this same strategy to React Native as well. All you need is a different `IndexedDB` implementation (or better yet, rename this to a more generic `Cache` service), and you're set!
